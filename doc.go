...
//
// Notify sends a single struct{}{} to every currently-registered listener.
// Delivery is concurrent but bounded by the Manager's concurrency limit.
// Each individual send is subject to the timeout passed to Notify; if a
...
// error wraps the cause of the listener's context (typically
// context.Canceled).
//
// # Error Handling
//
...
//   - A per-listener error slice, with one entry per listener that was
//     attempted. A nil entry means the notification was delivered
//     successfully; a non-nil entry contains a wrapped error (typically
//     context.DeadlineExceeded or context.Canceled) that can be inspected
//     with errors.Is.
//
// Sentinel errors are returned (wrapped) for conditions callers may want to
// inspect with [errors.Is]:
//
//   - [ErrNilContext] when a nil context is passed to NewListener or Notify
//   - [ErrDuplicateID] when NewListener is called with an ID that is already
//     registered
//
...
// notify, and errors.Is or errors.Unwrap to inspect an underlying cause such
// as context.Canceled or context.DeadlineExceeded.
//
...
//     times.
//   - Implicitly, when the context passed to NewListener is canceled. The
...
// # Concurrency Safety
//
// Both Manager and Listener are safe for concurrent use. NewListener,
// GetListenerByID, Notify, and Listener.Cancel may all be called from
