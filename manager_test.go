...
func TestManager_Notify(t *testing.T) {
	...
	t.Run("Normal", func(t *testing.T) {
		...
		case <-doneCh:
			...
			if len(data.results) != testConcurrency {
				t.Fatalf("expected %d return codes in `sendResults`, got %d", testConcurrency, len(data.results))
			}
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		case <-doneCh:
			...
			if len(data.results) != testConcurrency {
				t.Fatalf("expected %d results in `sendResults`, got %d", testConcurrency, len(data.results))
			}
			for _, r := range data.results {
				if r == nil {
	...
	t.Run("Canceled Notify Context", func(t *testing.T) {
		...
		case <-doneCh:
			...
			if len(data.results) > testConcurrency {
				t.Fatalf("expected at most %d results in `sendResults`, got %d", testConcurrency, len(data.results))
			}
			for _, r := range data.results {
				if r == nil {
					t.Error("expected an error")
