...
func (l *Listener) notify(ctx context.Context) error {
	...
	if l.canceled {
		return newError(fmt.Sprintf("notify listener %q: listener is canceled", l.id), nil)
	}
...
func (m *Manager) Notify(ctx context.Context, timeout time.Duration) (sendResults []error, err error) {
	if ctx == nil {
		err = ErrNilContext
		return
	}
...
sendingLoop:
	...
	close(errCh)
	for e := range errCh {
		sendResults = append(sendResults, e)
	}
	return
}
